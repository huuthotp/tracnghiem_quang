...
(function(j, b, d) {
	...
	var f = MathJax.Object
			.Subclass({
				timeout : (k ? 15 : 8) * 1000,
				comparisonFont : [ "sans-serif", "monospace", "script",
				...
				testFont : function(p) {
					...
					if (!p.isWebFont && !r.match(/^(STIX|MathJax)|'/)) {
						r = r.replace(/_/g, " ").replace(/([a-z])([A-Z])/g,
								"$1 $2").replace(/ Jax/, "Jax")
								+ "','" + r + "','" + r + "-";
						if (p.weight) {
							r += "Bold"
					...
					var o = this.getComparisonWidths(p.testString,
							p.noStyleChar);
					var s = null;
					if (o) {
						this.div.style.fontFamily = r + ","
								+ this.comparisonFont[0];
						if (this.div.offsetWidth == o[0]) {
							this.div.style.fontFamily = r + ","
									+ this.comparisonFont[o[2]];
						...
						if (s === null
							...
							if (!p.noStyleChar && d.FONTDATA
									&& d.FONTDATA.hasStyleChar) {
				...
				testStyleChar : function(p, r) {
					...
					if (s === u) {
						...
						p.version = Math.floor((this.div.offsetWidth - q) / m
								+ 1.5) / 2
					}
				...
				loadWebFont : function(o) {
					b.Startup.signal.Post("HTML-CSS Jax - Web-Font "
							+ d.fontInUse + "/" + o.directory);
					var q = e([ "LoadWebFont", "Loading web-font %1",
							d.fontInUse + "/" + o.directory ]);
					var m = MathJax.Callback({});
					var p = MathJax.Callback([ "loadComplete", this, o, q, m ]);
					j.timer.start(j, [ this.checkWebFont, o, p ], 0,
							this.timeout);
					return m
				},
				loadComplete : function(p, r, o, m) {
					...
					if (b.Browser.isFirefox && d.allowWebFonts) {
						var q = document.location.protocol + "//"
				...
				loadError : function(m) {
					e([ "CantLoadWebFont", "Can't load web font %1",
							d.fontInUse + "/" + m.directory ], null, 2000);
					b.Startup.signal.Post([ "HTML-CSS Jax - web font error",
							d.fontInUse + "/" + m.directory, m ])
				},
				firefoxFontError : function(m) {
					e(
							...
							null, 3000);
					b.Startup.signal
							.Post("HTML-CSS Jax - Firefox web fonts on remote host error")
				},
				...
				fontFace : function(q) {
					...
					var p = j.fileURL(m);
					var o = q.replace(/-b/, "-B").replace(/-i/, "-I").replace(
							/-Bold-/, "-Bold");
					if (!o.match(/-/)) {
						o += "-Regular"
					}
			...
			.Augment({
				...
				Config : function() {
					...
					if (m) {
						...
						if (this.imgFonts) {
							this.require
									.push(this.directory + "/imageFonts.js");
							b.Startup.signal
									.Post("HTML-CSS Jax - using image fonts")
						}
					} else {
						e([
								"CantFindFontUsing",
					...
					this.require.push(MathJax.OutputJax.extensionDir
							+ "/MathEvents.js")
				},
				Startup : function() {
					...
					if (!document.body.firstChild) {
						...
						document.body.insertBefore(this.hiddenDiv,
					...
					this.startMarker = this.createStrut(this.Element("span"),
							10, true);
					...
					return j.Styles(this.config.styles, [ "InitializeHTML",
							this ])
				},
					...
					j.timer
							.start(
									...
									function(n) {
										if (n.time(m)) {
											b.signal
													.Post([ "HTML-CSS Jax - no default em size" ]);
											return
										}
										d.getDefaultExEm();
										if (d.defaultEm) {
				...
				preTranslate : function(s) {
					...
					for (D = 0; D < z; D++) {
						...
						G.parentNode.insertBefore(
								this.EmExSpan.cloneNode(true), G);
						u.parentNode.insertBefore(this.linebreakSpan
								.cloneNode(true), u)
					}
					var B = [];
					for (D = 0; D < z; D++) {
						...
						A = Math.max(0,
								u.previousSibling.firstChild.offsetWidth - 2);
						if (F === 0 || F === "NaN") {
							B.push(u);
						...
						H = (this.config.matchFontHeight ? F
								/ this.TeX.x_height / o : 1);
						H = Math.floor(Math.max(
								this.config.minScaleAdjust / 100, H)
								* this.config.scale);
						...
						p.HTMLCSS.lineWidth = (q ? this.length2em(x, 1, v
								/ this.em) : 1000000)
					}
				...
				Translate : function(n, r) {
					...
					if (r.HTMLCSSdelay) {
						...
						b.RestartAfter(MathJax.Callback
								.Delay(this.config.EqnChunkDelay))
					}
					...
					this.restorePreview(n);
					s.className = s.className.split(/ /)[0]
							+ " MathJax_Processed";
					...
					if (r.HTMLCSSeqn >= r.HTMLCSSlast + r.HTMLCSSchunk) {
						...
						r.HTMLCSSchunk = Math.floor(r.HTMLCSSchunk
								* this.config.EqnChunkFactor);
						r.HTMLCSSdelay = true
					}
					return false
				},
					...
					for (s = o.HTMLCSSlast, q = o.HTMLCSSeqn; s < q; s++) {
						...
						if (v && v.parentNode && v.MathJax.elementJax) {
							...
							if (p.HTMLCSS.span && p.HTMLCSS.div) {
								p.root.toHTML(p.HTMLCSS.span, p.HTMLCSS.div,
										this.PHASE.II)
							}
						}
					}
					for (s = o.HTMLCSSlast, q = o.HTMLCSSeqn; s < q; s++) {
						...
						if (v && v.parentNode && v.MathJax.elementJax) {
							...
							if (p.HTMLCSS.span && p.HTMLCSS.div) {
								...
								b.signal.Post([ "New Math",
										v.MathJax.elementJax.inputID ])
							}
						}
					}
					if (this.forceReflow) {
				...
				Zoom : function(n, y, x, m, v) {
					...
					if (p) {
						...
						var o = (n.root.id || "MathJax-Span-" + n.root.spanID)
								+ "-zoom";
					...
					if (z.H != null && z.H > z.h) {
						r.marginTop = d.Em(z.H
								- Math.max(z.h, d.FONTDATA.lineH))
					}
					if (z.D != null && z.D > z.d) {
						r.marginBottom = d.Em(z.D
								- Math.max(z.d, d.FONTDATA.lineD))
					}
					if (z.lw < 0) {
				...
				getW : function(q) {
					...
					if (q.bbox && this.config.noReflows
							&& q.bbox.exactW !== false) {
						if (!q.bbox.exactW) {
							if (q.style.paddingLeft) {
								o += this.unEm(q.style.paddingLeft)
										* (q.scale || 1)
							}
					...
					if ((q.bbox && o >= 0 && !this.initialSkipBug && !this.msieItalicWidthBug)
							|| this.negativeBBoxes || !q.firstChild) {
						n = q.offsetWidth;
				...
				MeasureSpans : function(q) {
					...
					for (s = 0, p = q.length; s < p; s++) {
						...
						if (this.negativeBBoxes || !v.firstChild
					...
					for (s = 0, p = t.length; s < p; s++) {
						...
						w = v.bbox;
						u = this.parentNode(v);
						if ((w.w >= 0 && !this.initialSkipBug)
								|| this.negativeBBoxes || !v.firstChild) {
							o = v.offsetWidth;
							u.HH = u.offsetHeight / this.em
						} else {
							if (w.w < 0 && this.msieNegativeBBoxBug) {
								o = -v.offsetWidth, u.HH = u.offsetHeight
										/ this.em
							} else {
								o = t[s][2].offsetLeft
										- ((t[s][1] || {}).offsetLeft || 0)
				...
				Px : function(n) {
					n *= this.em;
					var o = (n < 0 ? "-" : "");
					return o + Math.abs(n).toFixed(1).replace(/\.?0+$/, "")
							+ "px"
				},
				unPx : function(n) {
				...
				length2em : function(t, o, r) {
					...
					var q = this.FONTDATA.TeX_factor, u = (d.zoomScale || 1)
							/ d.em;
					if (t.match(/mathspace$/)) {
						return d.MATHSPACE[t] * q
					}
					var p = t
							.match(/^\s*([-+]?(?:\.\d+|\d+(?:\.\d*)?))?(pt|em|ex|mu|px|pc|in|mm|cm|%)?/);
				...
				getPadding : function(n) {
					...
					for ( var q in p) {
						if (p.hasOwnProperty(q)) {
							var o = n.style["padding"
									+ q.charAt(0).toUpperCase() + q.substr(1)];
							if (o) {
				...
				getBorders : function(r) {
					...
					for ( var s in o) {
						if (o.hasOwnProperty(s)) {
							var m = "border" + s.charAt(0).toUpperCase()
									+ s.substr(1);
				...
				createRule : function(t, p, r, u, n) {
					...
					if (u > p + r) {
						...
						m.width = this.Em(u);
						m.height = (this.msieRuleBug && p + r > 0 ? this.Em(p
								+ r) : 0)
					} else {
				...
				createFrame : function(v, s, u, x, z, n) {
					...
					var y = this.Em(s + u - r), m = this.Em(-u - z), q = this
							.Em(x - r);
					var o = this.Px(z) + " " + n;
				...
				placeBox : function(v, u, s, p) {
					v.isMathJax = true;
					var w = d.parentNode(v), E = v.bbox, A = w.bbox;
					...
					if (!v.noAdjust) {
						...
						if (this.msieInlineBlockAlignBug) {
							...
							if (d.chromeHeightBug) {
								z -= (v.lastChild.offsetHeight - Math.round(z
										* this.em))
										/ this.em
							}
						}
					...
					if (E) {
						...
						if (!this.msieClipRectBug && !E.noclip && !p) {
							var C = 3 / this.em;
							var B = (E.H == null ? E.h : E.H), n = (E.D == null ? E.d
									: E.D);
							var F = z - B - C, q = z + n + C, o = -1000, m = E.rw + 1000;
							v.style.clip = "rect(" + this.Em(F) + " "
									+ this.Em(m) + " " + this.Em(q) + " "
									+ this.Em(o) + ")"
						}
					}
				...
				createDelimiter : function(w, n, p, s, q) {
					...
					while (o.alias) {
						...
						if (!o) {
							o = {
								HW : [ 0,
										this.FONTDATA.VARIANT[i.VARIANT.NORMAL] ]
							}
						}
					}
					if (o.load) {
						b.RestartAfter(j.Require(this.fontDir + "/fontdata-"
								+ o.load + ".js"))
					}
					for ( var u = 0, r = o.HW.length; u < r; u++) {
						if (o.HW[u][0] * s >= p - 0.01
								|| (u == r - 1 && !o.stretch)) {
							if (o.HW[u][2]) {
								s *= o.HW[u][2]
							}
				...
				extendDelimiterV : function(B, u, F, G, x) {
					...
					if (u > C) {
						...
						var D = o.bbox.h + o.bbox.d, m = D - 0.05, z, r, A = (F.mid ? 2
								: 1);
						r = z = Math.min(Math.ceil((u - C) / (A * m)),
					...
					B.bbox = {
						...
						lw : Math.min(w.bbox.lw, o.bbox.lw, v.bbox.lw,
								E.bbox.lw),
						rw : Math.max(w.bbox.rw, o.bbox.rw, v.bbox.rw,
								E.bbox.rw),
						h : 0,
						d : -s,
						exactW : true
				...
				extendDelimiterH : function(C, p, F, H, z) {
					...
					var v = (q.bbox.rw - q.bbox.lw) + (D.bbox.rw - D.bbox.lw)
							- 0.05, u = q.bbox.rw - q.bbox.lw - 0.025, y;
					if (F.mid) {
						E = this.createBox(s);
					...
					if (p > v) {
						var G = m.bbox.rw - m.bbox.lw, r = G - 0.05, A, t, B = (F.mid ? 2
								: 1);
						t = A = Math.min(Math.ceil((p - v) / (B * r)),
								this.maxStretchyParts);
						if (!F.fillExtenders) {
							r = (p - v) / (B * A)
						}
				...
				handleVariant : function(B, q, t) {
					...
					for ( var C = 0, z = t.length; C < z; C++) {
							...
							for (v = 0, s = w.length; v < s; v++) {
								...
								if (r && y >= w[v].low && y <= w[v].high) {
									...
									if (q["variant" + w[v].offset]) {
										q = this.FONTDATA.VARIANT[q["variant"
												+ w[v].offset]]
									}
									break
								}
						...
						if (p || (!this.checkFont(u, o.style) && !D[5].img)) {
							...
							var x = !!o.style.fontFamily || !!B.style.fontStyle
									|| !!B.style.fontWeight || !u.directory
									|| p;
							p = false;
							if (o !== B) {
				...
				checkFont : function(m, n) {
					...
					return (m.family.replace(/'/g, "") === n.fontFamily
							.replace(/'/g, "")
							&& (m.style || "normal") === (n.fontStyle || "normal") && (m.weight || "normal") === o)
				},
				handleFont : function(o, m, q) {
					o.style.fontFamily = m.family;
					if (!m.directory) {
						o.style.fontSize = Math.floor(d.config.scale / d.scale
								+ 0.5)
								+ "%"
					}
				...
				handleChar : function(o, m, u, t, s) {
					...
					if (r.c == null) {
						if (t <= 65535) {
							...
							var p = t - 65536;
							r.c = String.fromCharCode((p >> 10) + 55296)
				...
				loadWebFontError : function(n, m) {
					...
					n.isWebFont = false;
					if (this.config.imageFont
							&& this.config.imageFont === this.fontInUse) {
						this.imgFonts = true;
						b.Startup.signal
								.Post("HTML-CSS Jax - switch to image fonts");
						b.Startup.signal
								.Post("HTML-CSS Jax - using image fonts");
						e(
								[ "WebFontNotAvailable",
					...
					function() {
								...
								.Augment(
										{
											...
											HTMLmultiline : function() {
												i.mbase
														.HTMLautoloadFile("multiline")
											},
											HTMLcomputeBBox : function(s, r, q,
													n) {
												if (q == null) {
													q = 0
												}
											...
											HTMLaddAttributes : function(p) {
												if (this.attrNames) {
													...
													var r = (this.type === "mstyle" ? i.math.prototype.defaults
															: this.defaults);
											...
											HTMLspanElement : function() {
												...
												return document
														.getElementById((this.id || "MathJax-Span-"
																+ this.spanID)
																+ d.idPostfix)
											},
											HTMLhandleVariant : function(n, m,
													o) {
											...
											HTMLhandleColor : function(y) {
												...
												if ((A.mathbackground && A.mathbackground !== i.COLOR.TRANSPARENT)
														|| v || x) {
													var C = y.bbox, B = (C.exact ? 0
															: 1 / d.em), w = 0, u = 0, o = y.style.paddingLeft, s = y.style.paddingRight;
													if (this.isToken) {
														w = C.lw;
														u = C.rw - C.w
													}
													...
													if (d.msieInlineBlockAlignBug) {
														...
														if (v
																&& d.msieBorderWidthBug) {
															z += v.top
																	+ v.bottom;
															q += v.left
																	+ v.right
														}
														p.style.width = d.Em(t
																+ B);
														d
																.placeBox(
																		d
											...
											HTMLhandleSpace : function(q) {
												if (this.useMMLspacing) {
													...
													if (o.scriptlevel <= 0
														...
														if (o.lspace) {
															q.style.paddingLeft = d
																	.Em(o.lspace)
														}
														if (o.rspace) {
															q.style.paddingRight = d
																	.Em(o.rspace)
														}
													}
												} else {
											...
											HTMLgetScale : function() {
												...
												var o = 1, m = this.getValues(
														"scriptlevel",
														"fontsize");
												m.mathsize = (this.isToken ? this
														: this.Parent())
														.Get("mathsize");
												if (this.style) {
													var n = this
												...
												this.scale = o;
												this.mscale = d
														.length2em(m.mathsize);
												return o * this.mscale
											},
											HTMLgetMu : function(o) {
												var m = 1, n = this.getValues(
														"scriptlevel",
											...
											HTMLgetVariant : function() {
												...
												if (m.weight
														&& m.weight
																.match(/^\d+$/)) {
													m.weight = (parseInt(m.weight) > 600 ? "bold"
															: "normal")
												}
												var n = m.mathvariant;
												if (this.variantForm) {
						...
						i.chars.Augment({
							toHTML : function(p, o, n, q) {
								var t = this.data.join("").replace(
										/[\u2061-\u2064]/g, "");
								if (n) {
									t = n(t, q)
								}
								if (o.fontInherit) {
									var s = Math.floor(d.config.scale / d.scale
											+ 0.5)
											+ "%";
									d.addElement(p, "span", {
										style : {
						...
						i.mo
								.Augment({
									toHTML : function(x) {
										...
										var z = this.getValues("largeop",
												"displaystyle");
										if (z.largeop) {
											s = d.FONTDATA.VARIANT[z.displaystyle ? "-largeOp"
													: "-smallOp"]
										}
										var y = this.CoreParent(), r = (y
												&& y.isa(i.msubsup) && this !== y.data[y.base]), o = (r ? this.remapChars
												: null);
										if (A.length === 1
												&& y
												&& y.isa(i.munderover)
												&& this
														.CoreText(y.data[y.base]).length === 1) {
										...
										if (r
												&& A
														.match(/['`"\u00B4\u2032-\u2037\u2057]/)) {
											s = d.FONTDATA.VARIANT["-"
													+ d.fontInUse + "-variant"]
										}
										for ( var t = 0, q = this.data.length; t < q; t++) {
											if (this.data[t]) {
												this.data[t].toHTML(x, s,
														this.remap, o)
											}
										}
										if (!x.bbox) {
										...
										if (z.largeop) {
											var v = d.TeX.axis_height
													* this.scale * this.mscale;
											var n = (x.bbox.h - x.bbox.d) / 2
													- v;
											if (d.safariVerticalAlignBug
													&& x.lastChild.nodeName === "IMG") {
												x.lastChild.style.verticalAlign = d
														.Em(d
																.unEm(x.lastChild.style.verticalAlign || 0)
																/ d.em
																- n
																/ x.scale)
											} else {
												if (d.konquerorVerticalAlignBug
														&& x.lastChild.nodeName === "IMG") {
													x.style.position = "relative";
													x.lastChild.style.position = "relative";
													x.lastChild.style.top = d
															.Em(n / x.scale)
												} else {
													x.style.verticalAlign = d
															.Em(-n / x.scale)
												}
											}
											x.bbox.h -= n;
									...
									HTMLcanStretch : function(q) {
										...
										if (o
												&& o.isa(i.munderover)
												&& this
														.CoreText(o.data[o.base]).length === 1) {
											var p = o.data[o.over], n = o.data[o.under];
											if (p && this === p.CoreMO()
													&& o.Get("accent")) {
												r = d.FONTDATA.REMAPACCENT[r]
														|| r
											} else {
												if (n && this === n.CoreMO()
														&& o.Get("accentunder")) {
													r = d.FONTDATA.REMAPACCENTUNDER[r]
															|| r
												}
											}
										}
										r = d.FONTDATA.DELIMITERS[r
												.charCodeAt(0)];
										var m = (r && r.dir === q.substr(0, 1));
										this.forceStretch = (m && (this.Get(
												"minsize", true) || this.Get(
												"maxsize", true)));
										return m
									},
									HTMLstretchV : function(o, p, q) {
										this.HTMLremoveColor();
										var t = this.getValues("symmetric",
												"maxsize", "minsize");
										var r = this.HTMLspanElement(), u = this
												.HTMLgetMu(r), s;
										var n = this.HTMLgetScale(), m = d.TeX.axis_height
												* n;
										if (t.symmetric) {
										...
										t.maxsize = d.length2em(t.maxsize, u,
												r.bbox.h + r.bbox.d);
										t.minsize = d.length2em(t.minsize, u,
												r.bbox.h + r.bbox.d);
										s = Math.max(t.minsize, Math.min(
												t.maxsize, s));
										if (s != t.minsize) {
											s = [
													Math
															.max(
																	s
																			* d.TeX.delimiterfactor
																			/ 1000,
																	s
																			- d.TeX.delimitershortfall),
													s ]
										}
										r = this.HTMLcreateSpan(o);
										d.createDelimiter(r, this.data.join("")
												.charCodeAt(0), s, n);
										if (t.symmetric) {
											s = (r.bbox.h + r.bbox.d) / 2 + m
										} else {
											s = (r.bbox.h + r.bbox.d) * p
													/ (p + q)
										}
									...
									HTMLstretchH : function(q, m) {
										this.HTMLremoveColor();
										var o = this.getValues("maxsize",
												"minsize", "mathvariant",
												"fontweight");
										if ((o.fontweight === "bold" || parseInt(o.fontweight) >= 600)
												&& !this.Get("mathvariant",
														true)) {
											o.mathvariant = i.VARIANT.BOLD
										}
										var p = this.HTMLspanElement(), n = this
												.HTMLgetMu(p), r = p.scale;
										o.maxsize = d.length2em(o.maxsize, n,
												p.bbox.w);
										o.minsize = d.length2em(o.minsize, n,
												p.bbox.w);
										m = Math.max(o.minsize, Math.min(
												o.maxsize, m));
										p = this.HTMLcreateSpan(q);
										d.createDelimiter(p, this.data.join("")
												.charCodeAt(0), m, r,
												o.mathvariant);
						...
						i.mspace
								.Augment({
									toHTML : function(q) {
										...
										var p = d.length2em(o.height, n)
												* this.mscale, r = d.length2em(
												o.depth, n)
												* this.mscale, m = d.length2em(
												o.width, n)
												* this.mscale;
										d.createSpace(q, p, r, m,
												o.mathbackground, true);
										return q
									}
								});
						i.mphantom
								...
								.Augment({
									toHTML : function(u, o, m) {
										...
										if (this.data[0] != null) {
											...
											var v = this.getValues("height",
													"depth", "width", "lspace",
													"voffset"), t = 0, r = 0, w = this
													.HTMLgetMu(u);
											this.HTMLgetScale();
											if (v.lspace) {
												t = this.HTMLlength2em(p,
														v.lspace, w)
											}
											if (v.voffset) {
												r = this.HTMLlength2em(p,
														v.voffset, w)
											}
											d.placeBox(p, t, r);
											t /= this.mscale;
											...
											u.bbox = {
												...
												H : Math
														.max(
																(p.bbox.H == null ? -d.BIGDIMEN
																		: p.bbox.H
																				+ r),
																p.bbox.h + r),
												D : Math
														.max(
																(p.bbox.D == null ? -d.BIGDIMEN
																		: p.bbox.D
																				- r),
																p.bbox.d - r)
											};
											if (v.height !== "") {
												u.bbox.h = this.HTMLlength2em(
														p, v.height, w, "h", 0)
											}
											if (v.depth !== "") {
												u.bbox.d = this.HTMLlength2em(
														p, v.depth, w, "d", 0)
											}
											if (v.width !== "") {
												u.bbox.w = this.HTMLlength2em(
														p, v.width, w, "w", 0)
											}
											...
											u.bbox.exact = !!((this.data[0] && this.data[0].data.length == 0)
													|| q.exec(v.height)
													|| q.exec(v.width) || q
													.exec(v.depth));
									...
									HTMLlength2em : function(s, t, o, u, n) {
										...
										var q = String(t).match(
												/width|height|depth/);
										var r = (q ? s.bbox[q[0].charAt(0)]
												: (u ? s.bbox[u] : 0));
										var p = d.length2em(t, o, r
												/ this.mscale)
												* this.mscale;
						...
						i.mrow
								.Augment({
									HTMLlineBreaks : function(m) {
										...
										return (d.config.linebreaks.automatic && m.bbox.w > d.linebreakWidth)
												|| this.hasNewline()
									},
									...
									HTMLstretchV : function(o, n, p) {
										...
										this.data[this.core].HTMLstretchV(m, n,
												p);
						...
						i.mfrac
								.Augment({
									toHTML : function(F) {
										...
										var w = d.createBox(o), s = d
												.createBox(o);
										d.MeasureSpans([
												this.HTMLboxChild(0, w),
												this.HTMLboxChild(1, s) ]);
										var m = this.getValues("displaystyle",
												"linethickness", "numalign",
												"denomalign", "bevelled");
										var K = this.HTMLgetScale(), E = m.displaystyle;
										var J = d.TeX.axis_height * K;
										if (m.bevelled) {
											var I = (E ? 0.4 : 0.15);
											var x = Math.max(w.bbox.h
													+ w.bbox.d, s.bbox.h
													+ s.bbox.d)
													+ 2 * I;
											var G = d.createBox(o);
											...
											d.placeBox(w, 0,
													(w.bbox.d - w.bbox.h) / 2
															+ J + I);
											d.placeBox(G, w.bbox.w - I / 2,
													(G.bbox.d - G.bbox.h) / 2
															+ J);
											d.placeBox(s, w.bbox.w + G.bbox.w
													- I, (s.bbox.d - s.bbox.h)
													/ 2 + J - I)
										} else {
											var n = Math
													.max(w.bbox.w, s.bbox.w);
											var A = d
													.thickness2em(
															m.linethickness,
															this.scale)
													* this.mscale, C, B, z, y;
											var D = d.TeX.min_rule_thickness
													/ this.em;
											if (E) {
												z = d.TeX.num1;
												y = d.TeX.denom1
											...
											d.alignBox(w, m.numalign, z, 0,
													true);
											d.alignBox(s, m.denomalign, -y, 0,
													true)
										}
									...
									HTMLhandleSpace : function(n) {
										if (!this.texWithDelims
												&& !this.useMMLspacing) {
											var o = d.TeX.nulldelimiterspace
													* this.mscale;
											var m = n.childNodes[d.msiePaddingWidthBug ? 1
													: 0].style;
											m.marginLeft = m.marginRight = d
													.Em(o);
											n.bbox.w += 2 * o;
											n.bbox.rw += 2 * o
										}
										this.SUPER(arguments).HTMLhandleSpace
												.call(this, n)
									}
								});
						i.msqrt
								.Augment({
									toHTML : function(z) {
										...
										n = Math.max(C + o / 4, 1.5
												* d.TeX.min_rule_thickness
												/ this.em);
										...
										y = this.HTMLaddRoot(B, v, y, v.bbox.h
												+ v.bbox.d - A, u);
										d.placeBox(v, y, A - v.bbox.h);
										d.placeBox(w, y + v.bbox.w, A
												- w.bbox.h + n);
						...
						i.mroot
								.Augment({
									...
									HTMLaddRoot : function(u, n, s, q, m) {
										...
										var p = this.HTMLrootHeight(n.bbox.h
												+ n.bbox.d, m, o)
												- q;
									...
									HTMLrootHeight : function(o, n, m) {
										return 0.45 * (o - 0.9 * n) + 0.6 * n
												+ Math.max(0, m.bbox.d - 0.075)
									}
								});
						i.mfenced
								.Augment({
									...
									HTMLcomputeBBox : function(r, q) {
										var o = r.bbox = {}, s = [];
										this.HTMLcheckStretchy(this.data.open,
												o, s, q);
										this.HTMLcheckStretchy(this.data[0], o,
												s, q);
										for ( var p = 1, n = this.data.length; p < n; p++) {
											if (this.data[p]) {
												this.HTMLcheckStretchy(
														this.data["sep" + p],
														o, s, q);
												this.HTMLcheckStretchy(
														this.data[p], o, s, q)
										...
										this.HTMLcheckStretchy(this.data.close,
												o, s, q);
						...
						i.munderover
								.Augment({
									toHTML : function(M, I, G) {
										var n = this.getValues("displaystyle",
												"accent", "accentunder",
												"align");
										...
										if (!n.displaystyle
												...
												&& (q.movablelimits || q
														.CoreMO()
														.Get("movablelimits"))) {
											return i.msubsup.prototype.toHTML
													.call(this, M)
										}
										...
										var F = d.TeX.rule_thickness
												* this.mscale, H = d.FONTDATA.TeX_factor;
											...
											if (this.data[N] != null) {
												...
												if (N == this.over) {
													if (C) {
														...
														B = d.TeX.big_op_spacing1
																* Q * H;
														A = d.TeX.big_op_spacing3
																* Q * H;
														K = Math
																.max(
																		B,
													...
													if (N == this.under) {
														if (C) {
															...
															B = d.TeX.big_op_spacing2
																	* Q * H;
															A = d.TeX.big_op_spacing4
																	* Q * H;
															K = Math.max(B, A
																	- z.bbox.h)
														}
						...
						i.msubsup
								.Augment({
									toHTML : function(M, K, F) {
										M = this.HTMLcreateSpan(M);
										var P = this.HTMLgetScale(), J = this
												.HTMLgetMu(M);
										var y = d.createStack(M), n, w = [];
										var x = d.createBox(y);
										if (this.data[this.base]) {
											w.push(this.data[this.base]
													.toHTML(x));
											if (F != null) {
												this.data[this.base]
														.HTMLstretchV(x, K, F)
										...
										var N = d.TeX.x_height * P, E = d.TeX.scriptspace
												* P * 0.75;
										var m, z;
										if (this
												.HTMLnotEmpty(this.data[this.sup])) {
											m = d.createBox(y);
											w.push(this.data[this.sup]
													.toHTML(m))
										}
										if (this
												.HTMLnotEmpty(this.data[this.sub])) {
											z = d.createBox(y);
											w.push(this.data[this.sub]
													.toHTML(z))
										}
										d.MeasureSpans(w);
										if (m) {
											m.bbox.w += E;
											m.bbox.rw = Math.max(m.bbox.w,
													m.bbox.rw)
										}
										if (z) {
											z.bbox.w += E;
											z.bbox.rw = Math.max(z.bbox.w,
													z.bbox.rw)
										}
										d.placeBox(x, 0, 0);
										var o = P;
										if (m) {
											o = this.data[this.sup]
													.HTMLgetScale()
										} else {
											if (z) {
												o = this.data[this.sub]
														.HTMLgetScale()
											}
										}
										var H = d.TeX.sup_drop * o, G = d.TeX.sub_drop
												* o;
										...
										var L = this.getValues(
												"subscriptshift",
												"superscriptshift");
										L.subscriptshift = (L.subscriptshift === "" ? 0
												: d.length2em(L.subscriptshift,
														J));
										L.superscriptshift = (L.superscriptshift === "" ? 0
												: d.length2em(
														L.superscriptshift, J));
										if (!m) {
											if (z) {
												A = Math.max(A, d.TeX.sub1 * P,
														z.bbox.h - (4 / 5) * N,
														L.subscriptshift);
												d.placeBox(z, x.bbox.w, -A,
														z.bbox)
											}
										} else {
											if (!z) {
												n = this.getValues(
														"displaystyle",
														"texprimestyle");
												I = d.TeX[(n.displaystyle ? "sup1"
														: (n.texprimestyle ? "sup3"
																: "sup2"))];
												B = Math.max(B, I * P, m.bbox.d
														+ (1 / 4) * N,
														L.superscriptshift);
												d.placeBox(m, x.bbox.w + O, B,
														m.bbox)
											} else {
												A = Math.max(A, d.TeX.sub2 * P);
												var C = d.TeX.rule_thickness
														* P;
												if ((B - m.bbox.d)
														- (z.bbox.h - A) < 3 * C) {
													A = 3 * C - B + m.bbox.d
								...
								.Augment({
									toHTML : function(E, B, t) {
										...
										if (!t || t === d.PHASE.III) {
											d.placeBox(w, 0, 0);
											var q = r.bbox.w;
											q = Math.abs(q) < 0.006 ? 0 : Math
													.max(0, Math.round(q
															* this.em) + 0.25);
											E.style.width = d.EmRounded(q
													/ d.outerEm);
											...
											if (!this.isMultiline
												...
												if (this.displayIndent !== "0") {
													var y = d.length2em(
															...
															1, d.scale
																	* d.cwidth);
													F += (o.indentalign === i.INDENTALIGN.RIGHT ? -y
															: y)
												}
												...
												if (F) {
													...
													if (D) {
														var v = parseFloat(D.style.marginLeft
																|| "0")
																+ F, s = parseFloat(D.style.marginRight
																|| "0")
																- F;
														D.style.marginLeft = d
																.Em(v);
														D.style.marginRight = d
																.Em(s
																		+ (o.indentalign === "right" ? E.bbox.w
																				+ F
																				- E.bbox.w
																				: 0));
														if (d.msieColorBug
							...
							toHTML : function(q, o, s) {
								...
								if (this.data[0] != null) {
									if (this.texClass === i.TEXCLASS.VCENTER) {
										...
										if (s != null) {
											p = this.data[0].HTMLstretchV(r, o,
													s)
										} else {
											if (o != null) {
												p = this.data[0].HTMLstretchH(
														r, o)
											}
										}
										q.bbox = p.bbox
						...
						b.Register.StartupHook("onLoad", function() {
							setTimeout(MathJax.Callback([ "loadComplete", d,
									"jax.js" ]), 0)
						})
					});
	b.Register
								...
								.Select({
									...
									Firefox : function(n) {
										...
										if (n.versionAtLeast("3.5")) {
											var m = String(document.location)
													.replace(/[^\/]*$/, "");
									...
									Safari : function(r) {
										...
										var n = (navigator.appVersion
												.match(/ Android (\d+)\.(\d+)/));
										var s = (o && r.isMobile && ((navigator.platform
												.match(/iPad|iPod|iPhone/) && !r
												.versionAtLeast("5.0")) || (n != null && (n[1] < 2 || (n[1] == 2 && n[2] < 2)))));
										d
												.Augment({
													config : {
														styles : {
													...
													forceReflow : true,
													FontFaceBug : true,
													allowWebFonts : (o && !s ? "otf"
															: false)
												});
										if (m) {
											d
													.Augment({
														webFontDefault : (r.isMobile ? "sans-serif"
																: "serif")
													})
										}
										if (r.isPC) {
									...
									Chrome : function(m) {
										d
												.Augment({
													...
													forceReflow : true,
													allowWebFonts : (m
															.versionAtLeast("4.0") ? "otf"
															: "svg")
												})
									},
									Opera : function(m) {
										m.isMini = (navigator.appVersion
												.match("Opera Mini") != null);
